...
def app():
    ...
    if x == "by price":
        ...
        #create radio buttons for selecting the budget
    ...
    #if price if beow 10 lakhs
    if price == "below 10 lakhs":
        def func1(x):
            #store the dataframe where carsrange is under budget
            df1 = df.loc[df['carsrange'] == 'Budget'][x].unique()
            df1 = pd.DataFrame(df1, columns=[x]) 
            return df1 #return the column x where carsrange is under budget 
    
    #if price if between 10 - 50 lakhs
    if price == "between 10 - 50 lakhs":
        def func2(x):
            #store the dataframe where carsrange is between 10 - 50 lakhs
            df2 = df.loc[df['carsrange'] == 'Medium'][x].unique()
            df2 = pd.DataFrame(df2, columns=[x])
            return df2 #return the column x where carsrange is between 10 - 50 lakhs
    #if price if between 50 lakhs - 1 cr
    if price == "between 50 lakhs - 1 cr":
        def func3(x):
            #store the dataframe where carsrange is between 50 lakhs - 1 cr
            df3 = df.loc[df['carsrange'] == 'Medium_High'][x].unique()
            df3 = pd.DataFrame(df3, columns=[x])
            return df3 #return the column x where carsrange is between 50 lakhs - 1 cr
   
    #if price if above 1cr 
    if price == "above 1cr":
         #store the dataframe where carsrange is above 1cr
        def func4(x):
            df4 = df.loc[df['carsrange'] == 'Highend'][x].unique()
            df4 = pd.DataFrame(df4, columns=[x])
            return df4 #return the column x where carsrange is above 1cr
    ...
    with details:
        ...
        if x == "by price":
            ...
            def selected_car(x):
                ...
                return df1
           #dataframe to store the summary(like - price, fuel_type etc) of the  cars 
            data_matrix = [['Features', brand],
                           ...
                           ['Mileage(km/l)', selected_car('Mileage')],
                           ['Fuel Type', selected_car('Fuel_Type'), ]
            ]
            
             #store the summary of  cars in an expander               
    ...
    if x == "by make":
        ...
        with features:
            ...
            new_df1 = (df1[['Model', 'Variant', 'Body_Type',
                            'Ex_Showroom_Price', 'Fuel_Type','Engine_Type','Power','Mileage']])
            AgGrid(new_df1)#list of car details of the selected brand

    global body 
    #sort the cars by body typ
    if x == "by body_type":
